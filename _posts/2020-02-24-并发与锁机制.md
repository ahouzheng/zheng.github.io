...
- 实现防止死锁，可以定时检查数据库数据，哥一定时间清除超时数据  
...
## 如何解决网站的高并发问题
- Redis缓存，将访问量大，而变化比较少的数据，放入Redis缓存
- 将访问量大的网页做静态化处理，可以使用Freemaker，访问静态页面无需经过Tomcat，可以直接被访问
- Tomcat集群
- Nginx动静分离
...
Channel的主要实现类有：FileChannel读写文件数据的通道，DatagramChannel通过UDP读写数据的通道，SocketChannel通过TCP读写网络数据的通道，ServerSocketChannel监听新进来的TCP连接，为每一个连接新建一个SocketChannel  
