...
map.stream().sorted(new Comparator<Map.Entry<Integer, People>>(){
    @Override
    public int compareTo(Map.Entry<Integer, People> a, Map.Entry<Integer, People> b){
...
**应用场景** 反射是Java框架的基石，在框架中大量使用了反射机制。  
例如:
